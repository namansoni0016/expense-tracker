...
import { fetchACategory } from "../../services/category/categoryService";
...
const UpdateCategory = () => {
    ...
    const formik = useFormik({
        initialValues: {
            type: data?.categoryFound?.type || "",
            name: data?.categoryFound?.name || "",
        },
    ...
    return (
        <form onSubmit={formik.handleSubmit} className="max-w-lg mx-auto my-10 bg-white p-6 rounded-lg shadow-lg space-y-6">
            ...
            <div className="space-y-2">
                ...
                <select id="type" {...formik.getFieldProps("type")} className="w-full p-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
            ...
            <div className="flex flex-col">
                ...
                <input type="text" {...formik.getFieldProps("name")} placeholder="Name" id="name" className="w-full mt-1 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 py-2 px-3"/>
                {formik.touched.name && formik.errors.name && (<p className="text-red-500 text-xs italic">{formik.errors.name}</p>)}
            </div>
            {/* Submit button */}
            <button type="submit" className="mt-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition-colors duration-200 transform">Update Category</button>
        </form>
    )
};
